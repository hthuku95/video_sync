...
impl VoyageEmbeddings {
    ...
    pub async fn generate_embeddings(&self, texts: Vec<String>) -> Result<Vec<Vec<f32>>, String> {
        self.generate_embeddings_with_type(texts, Some(InputType::Document))
            .await
    }
    ...
    pub async fn embed_document(&self, text: String) -> Result<Vec<f32>, String> {
        let embeddings = self
            .generate_embeddings_with_type(vec![text], Some(InputType::Document))
            .await?;
        embeddings
            .into_iter()
            .next()
    ...
    pub async fn embed_query(&self, text: String) -> Result<Vec<f32>, String> {
        let embeddings = self
            .generate_embeddings_with_type(vec![text], Some(InputType::Query))
            .await?;
        embeddings
            .into_iter()
            .next()
