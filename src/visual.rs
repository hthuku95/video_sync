...
pub fn color_balance(
    ...
    output_file: &str,
    shadows: (f64, f64, f64),
    midtones: (f64, f64, f64),
    highlights: (f64, f64, f64),
) -> Result<String, String> {
    let filter = format!(
...
pub fn reduce_noise(
    ...
    let filter = format!(
        "nlmeans=s={}:r={}:p={}",
        strength, research_size, patch_size
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn blend_videos(
    ...
    let filter = format!(
        "[0:v][1:v]blend=all_mode={}:all_opacity={}",
        blend_mode, opacity
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn zoompan(
    ...
    let filter = format!(
        "zoompan=z='{}':x='{}':y='{}':d={}:fps={}",
        zoom, x_expr, y_expr, duration_frames, fps
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn adjust_curves(
    ...
    let filter = if !preset.is_empty() {
        ...
        if parts.is_empty() {
            return Err("adjust_curves: provide preset or at least one channel curve".to_string());
        }
...
pub fn convert_colorspace(
    ...
    if !trc.is_empty() {
        parts.push(format!("trc={}", trc));
    }
    if !primaries.is_empty() {
        parts.push(format!("primaries={}", primaries));
    }
...
pub fn apply_shear(
    ...
    let filter = format!(
        "shear=shx={}:shy={}:fillcolor={}:interp={}",
        shx, shy, fillcolor, interp
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn tile_frames(
    ...
    let filter = format!(
        "tile={}x{}:padding={}:margin={}",
        columns, rows, frame_gap, frame_gap
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn adjust_color_temperature(
    input_file: &str,
    output_file: &str,
    temperature: f64,
    mix: f64,
...
pub fn adjust_vibrance(
    ...
    let filter = format!(
        "vibrance=intensity={}:rbal={}:gbal={}:bbal={}",
        intensity, rbal, gbal, bbal
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn draw_grid(
    ...
    let filter = format!(
        "drawgrid=width={}:height={}:thickness={}:color={}",
...
pub fn decimate_frames(
    ...
    let filter = format!(
        "decimate=cycle={}:dupthresh={}:scthresh={}",
        cycle, dupthresh, scthresh
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn denoise_video_owden(
    ...
    let filter = format!(
        "owdenoise=luma_strength={}:chroma_strength={}",
...
pub fn detect_objects_dnn(
    ...
    let filter = if labels.is_empty() {
        format!(
            "dnn_detect=dnn_backend={}:model={}:confidence={}",
            backend, model, confidence
        )
    } else {
        format!(
            "dnn_detect=dnn_backend={}:model={}:confidence={}:labels={}",
            backend, model, confidence, labels
        )
    };
    let mut command = Command::new("ffmpeg");
    command
...
pub fn classify_frames_dnn(
    ...
    let filter = if labels.is_empty() {
        ...
        format!(
            "dnn_classify=dnn_backend={}:model={}:labels={}",
...
pub fn upscale_super_resolution(
    input_file: &str,
    output_file: &str,
    scale_factor: u32,
    model: &str,
    ...
    let filter = if model.is_empty() {
        ...
        format!(
            "sr=dnn_backend={}:scale_factor={}:model={}",
            backend, scale_factor, model
        )
    };
    let mut command = Command::new("ffmpeg");
    command
...
pub fn zoom_pan(
    ...
    let x = if x_expr.is_empty() {
        "iw/2-(iw/zoom/2)".to_string()
    } else {
        x_expr.to_string()
    };
    let y = if y_expr.is_empty() {
        "ih/2-(ih/zoom/2)".to_string()
    } else {
        y_expr.to_string()
    };
    let d = if duration_frames == 0 {
        125
    } else {
        duration_frames
    };
...
pub fn chromatic_aberration(
    ...
    let filter = format!(
        "rgbashift=rh={}:rv={}:gh=0:gv=0:bh={}:bv={}",
        rh, rv, bh, bv
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn temporal_blend(
    ...
    let op = if opacity <= 0.0 || opacity > 1.0 {
        1.0
    } else {
        opacity
    };
...
pub fn motion_interpolate(
    ...
    let filter = format!(
        "minterpolate=fps={}:mi_mode={}:mc_mode=aobmc:me_mode=bidir:vsbmc=1",
        fps_val, mode
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn correct_perspective_linear(
    input_file: &str,
    output_file: &str,
    x0: f64,
    y0: f64,
...
pub fn denoise_hqdn3d(
    ...
    let ls = if luma_spatial <= 0.0 {
        4.0
    } else {
        luma_spatial
    };
    let cs = if chroma_spatial <= 0.0 {
        3.0
    } else {
        chroma_spatial
    };
...
pub fn apply_erosion(
    ...
    let coords = if coordinates == 0 { 255 } else { coordinates };
    let filter = format!(
        "erosion=threshold0={}:threshold1={}:threshold2={}:threshold3={}:coordinates={}",
        threshold0, threshold1, threshold2, threshold3, coords
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn apply_histogram_eq(
    ...
    let i = if intensity <= 0.0 {
        0.21
    } else {
        intensity.min(1.0)
    };
    let ab = if antibanding.is_empty() {
        "none"
    } else {
        antibanding
    };
...
pub fn apply_deblock(
    ...
    let p = if planes == 0 { 15 } else { planes };
    let filter = format!(
        "deblock=filter={}:block={}:alpha={}:beta={}:gamma={}:delta={}:planes={}",
        ft, bs, s, s, s, s, p
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn adjust_hue_saturation(
    ...
    let filter = format!(
        "huesaturation=hue={}:saturation={}:intensity={}:lightness={}",
        hue, sat, ints, luma
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn apply_smart_blur(
    ...
    let ls = if luma_strength.abs() < f64::EPSILON {
        -0.3
    } else {
        luma_strength
    };
...
pub fn add_film_grain(
    ...
    let s = if all_strength == 0 {
        8
    } else {
        all_strength.min(100)
    };
...
pub fn apply_geq(
    ...
    let l = if lum_expr.is_empty() {
        "lum(X,Y)"
    } else {
        lum_expr
    };
    let cb = if cb_expr.is_empty() {
        "cb(X,Y)"
    } else {
        cb_expr
    };
    let cr = if cr_expr.is_empty() {
        "cr(X,Y)"
    } else {
        cr_expr
    };
...
pub fn apply_colorchannelmixer(
    input_file: &str,
    output_file: &str,
    rr: f64,
    rg: f64,
...
pub fn apply_atadenoise(
    ...
    let w = if window_size == 0 {
        9
    } else {
        (window_size | 1).min(129)
    };
    let ta = if threshold_a <= 0.0 {
        0.02
    } else {
        threshold_a
    };
    let tb = if threshold_b <= 0.0 {
        0.04
    } else {
        threshold_b
    };
    ...
    let filter = format!(
        "atadenoise=0a={}:0b={}:1a={}:1b={}:2a={}:2b={}:s={}:p={}",
        ta, tb, ta, tb, ta, tb, w, p
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn apply_vaguedenoiser(
    ...
    let p = if planes == 0 { 7 } else { planes };
    let filter = format!(
        "vaguedenoiser=threshold={}:method={}:nsteps={}:percent={}:planes={}",
        t, m, n, pc, p
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn apply_fftdnoiz(
    ...
    let p = if planes == 0 { 7 } else { planes };
    let filter = format!(
        "fftdnoiz=sigma={}:amount={}:block={}:overlap={}:planes={}",
        s, a, bs, ov, p
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn apply_amplify(
    ...
    let p = if planes == 0 { 7 } else { planes };
    let filter = format!(
        "amplify=radius={}:factor={}:threshold={}:planes={}",
        r, f, t, p
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn apply_shuffleplanes(
    ...
    let filter = format!(
        "shuffleplanes=map0={}:map1={}:map2={}:map3={}",
        map0, map1, map2, map3
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn detect_black_frames(
    input_file: &str,
    black_min_duration: f64,
    picture_black_ratio_th: f64,
    pixel_black_th: f64,
) -> Result<String, String> {
    let d = if black_min_duration <= 0.0 {
        2.0
    } else {
        black_min_duration
    };
    let pbt = if pixel_black_th <= 0.0 {
        0.10
    } else {
        pixel_black_th
    };
    let pbr = if picture_black_ratio_th <= 0.0 {
        0.98
    } else {
...
pub fn detect_interlace_type(input_file: &str) -> Result<String, String> {
    ...
    let mut lines = Vec::new();
    for line in stderr.lines() {
        if line.contains("idet")
            || line.contains("TFF")
            || line.contains("BFF")
            || line.contains("Progressive")
...
pub fn apply_stereo3d(
    ...
    let inf = if input_format.is_empty() {
        "sbsl"
    } else {
        input_format
    };
    let outf = if output_format.is_empty() {
        "arcd"
    } else {
        output_format
    };
...
pub fn apply_maskedclamp(
    ...
    let p = if planes == 0 { 15 } else { planes };
    let filter = format!(
        "[0:v][1:v][2:v]maskedclamp=undershoot={}:overshoot={}:planes={}[vout]",
        undershoot, overshoot, p
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn apply_datascope(
    ...
    let filter = format!(
        "datascope=size={}:x={}:y={}:mode={}:opacity={}:axis={}",
        s,
        x,
        y,
        mode,
...
pub fn apply_fspp(
    ...
    let filter = format!(
        "fspp=quality={}:strength={}:use_bframe_qp={}",
        q,
        strength,
        if use_bframe_qp { 1 } else { 0 }
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn apply_nlmeans_video(
    ...
    let filter = format!(
        "nlmeans=s={}:p={}:pc={}:r={}:rc={}",
        strength, patch, patch_c, research, research_c
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn apply_median_spatial(
    ...
    let p = if planes == 0 { 15 } else { planes };
    let filter = format!(
        "median=radius={}:radiusV={}:percentile={}:planes={}",
        r, rv, percentile, p
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn convert_360_video(
    ...
    let inf = if input_fmt.is_empty() {
        "equirect"
    } else {
        input_fmt
    };
    let outf = if output_fmt.is_empty() {
        "flat"
    } else {
        output_fmt
    };
    ...
    let filter = format!(
        "v360={}:{}:w={}:h={}:h_fov={}:v_fov={}",
        inf, outf, w, h, h_fov, v_fov
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn apply_greyedge(
    ...
    let filter = format!(
        "greyedge=difford={}:minknorm={}:sigma={}",
        difford, minknorm, sigma
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn apply_fillborders(
    ...
    let filter = if m == "fixed" {
        format!(
            "fillborders=left={}:right={}:top={}:bottom={}:mode={}:color={}",
            left, right, top, bottom, m, color
        )
    } else {
        format!(
...
pub fn apply_chromanr(
    ...
    let filter = format!(
        "chromanr=thres={}:sizew={}:sizeh={}:stepw={}:steph={}",
        t, sw, sh, spw, sph
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn apply_weave(
    ...
    let ff = if first_field.is_empty() {
        "top"
    } else {
        first_field
    };
...
pub fn apply_hsv_key(
    ...
    let filter = format!(
        "hsvkey=hue={}:sat={}:val={}:similarity={}:blend={}",
        hue, saturation, value, similarity, blend
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn apply_freezeframes(
    ...
    let filter = format!(
        "[0:v][1:v]freezeframes=first={}:last={}:replace={}[vout]",
        first, last, replace
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn stabilize_video_2pass(
    ...
    let pass1_filter = format!(
        "vidstabdetect=shakiness={}:accuracy={}:result='{}'",
        shak, acc, transform_file
    );
    ...
    let pass2_filter = format!(
        "vidstabtransform=smoothing={}:zoom={}:input='{}'",
        smth, z, transform_file
    );
...
pub fn apply_hsvhold(
    ...
    let filter = format!(
        "hsvhold=hue={}:sat={}:val={}:similarity={}:blend={}",
        hue, sat, val, sim, blend
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
