...
pub fn youtube_ready_export(input_file: &str, output_file: &str) -> Result<String, String> {
    ...
    crate::visual::stabilize_video_2pass(input_file, &t1, 5, 10, 10, 0.0)
        .inspect_err(|_| { cleanup(&t1); })?;

    // Pass 2: color grading (normalize)
    crate::visual::normalize_video(&t1, &t2, 0u32)
        .inspect_err(|_| { cleanup(&t1); cleanup(&t2); })?;
    cleanup(&t1);

    // Pass 3: loudnorm to -14 LUFS (YouTube target)
    crate::audio::normalize_loudness(&t2, &t3, -14.0, 11.0, -1.0)
        .inspect_err(|_| { cleanup(&t2); cleanup(&t3); })?;
    cleanup(&t2);

    // Pass 4: convert to yuv420p for maximum compatibility
    crate::visual::convert_pixel_format(&t3, output_file, "yuv420p")
        .inspect_err(|_| { cleanup(&t3); })?;
    cleanup(&t3);
...
pub fn podcast_cleanup(input_file: &str, output_file: &str) -> Result<String, String> {
    ...
    crate::audio::denoise_audio(input_file, &t1, -30.0, 12.0, false)
        .inspect_err(|_| { cleanup(&t1); })?;

    // Pass 2: reduce sibilance (de-esser at 8.5 kHz)
    crate::audio::reduce_sibilance(&t1, &t2, 8500.0, 0.3, "split")
        .inspect_err(|_| { cleanup(&t1); cleanup(&t2); })?;
    cleanup(&t1);

    // Pass 3: limit peaks to -1 dB
    crate::audio::audio_limiter(&t2, &t3, -1.0, 5.0, 50.0, false)
        .inspect_err(|_| { cleanup(&t2); cleanup(&t3); })?;
    cleanup(&t2);

    // Pass 4: loudnorm to -16 LUFS (podcast standard)
    crate::audio::normalize_loudness(&t3, output_file, -16.0, 11.0, -1.5)
        .inspect_err(|_| { cleanup(&t3); })?;
    cleanup(&t3);
...
pub fn cinematic_grade(input_file: &str, output_file: &str) -> Result<String, String> {
    ...
    crate::visual::adjust_curves(input_file, &t1, "vintage", "", "", "", "")
        .inspect_err(|_| { cleanup(&t1); })?;

    // Pass 2: boost vibrance (intensity=0.4, neutral RGB balance)
    crate::visual::adjust_vibrance(&t1, &t2, 0.4, 0.0, 0.0, 0.0)
        .inspect_err(|_| { cleanup(&t1); cleanup(&t2); })?;
    cleanup(&t1);

    // Pass 3: add vignette (angle=PI/5, forward mode)
    crate::visual::add_vignette(&t2, &t3, std::f64::consts::PI / 5.0, "forward")
        .inspect_err(|_| { cleanup(&t2); cleanup(&t3); })?;
    cleanup(&t2);

    // Pass 4: film grain
    crate::visual::add_film_grain(&t3, output_file, 15, "")
        .inspect_err(|_| { cleanup(&t3); })?;
    cleanup(&t3);
...
pub fn create_gif(
    ...
    crate::core::trim_video(input_file, &t1, start_seconds, end)
        .inspect_err(|_| { cleanup(&t1); })?;

    // Pass 2: scale + optimize palette → direct GIF output
    crate::visual::optimize_gif_palette(&t1, output_file, width, fps, "diff")
        .inspect_err(|_| { cleanup(&t1); })?;
    cleanup(&t1);
...
pub fn talking_head_cleanup(input_file: &str, output_file: &str) -> Result<String, String> {
    ...
    crate::audio::denoise_audio(&t1, &t2, -30.0, 12.0, false)
        .inspect_err(|_| { cleanup(&t1); cleanup(&t2); })?;
    cleanup(&t1);

    // Pass 3: reduce sibilance
    crate::audio::reduce_sibilance(&t2, &t3, 8500.0, 0.3, "split")
        .inspect_err(|_| { cleanup(&t2); cleanup(&t3); })?;
    cleanup(&t2);
