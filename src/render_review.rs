...
pub async fn review_render(
    ...
    let gemini = match state.video_gemini_client.as_ref()
        .or(state.gemini_client.as_ref())
    {
        Some(g) => g,
        None => return ReviewResult {
...
Score ≥6 = ready to ship. Score ≤5 = needs re-render.

If score ≤5, give a CONCRETE retry hint — one sentence the caller can prepend to the prompt to fix the specific issue (e.g. "use a taller aspect ratio", "avoid small text", "slow down the camera movement").

Return ONLY JSON, no markdown:
{{"score": 8, "feedback": "Clean composition, readable title, matches the podcast vibe.", "retry_hint": null}}
or
{{"score": 4, "feedback": "Text too small to read on mobile.", "retry_hint": "Use 40pt+ title font and center it in the upper third."}}"#,
        original_prompt = original_prompt,
        tool_name       = tool_name,
        output_url      = output_url,
    );

    let response = match gemini.generate_text(&prompt).await {
        Ok(r) => r,
        Err(e) => {
            return ReviewResult {
    ...
    let parsed: serde_json::Value = match serde_json::from_str(cleaned) {
        ...
        Err(_) => {
            return ReviewResult {
                pass: true,
                score: 0,
                feedback: format!("Unparseable review response: {}", cleaned.chars().take(120).collect::<String>()),
                retry_hint: None,
            };
        }
    };
    ...
    let feedback = parsed.get("feedback").and_then(|f| f.as_str()).unwrap_or("").to_string();
    let retry_hint = parsed.get("retry_hint")
        .and_then(|h| h.as_str())
        .filter(|s| !s.is_empty())
        .map(|s| s.to_string());
