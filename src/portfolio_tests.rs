//! Portfolio test runner — DFY service scenarios via AgenticServicePipeline.
//!
//! Triggered from /admin/test-runs. Each run exercises all 12 DFY services
...
impl PortfolioTestRunner {
    ...
    /// Create a new test_run row and spawn a background task that runs all 12 DFY services.
    ...
    async fn run_all(&self, run_id: Uuid) {
        ...
        tracing::info!("Portfolio test run {run_id} finished (12 DFY services)");
    }

    async fn run_one(&self, run_id: Uuid, scenario: &DfyScenario) {
        tracing::info!("▶ DFY test: {}", scenario.name);
    ...
    /// Run a DFY service through the AgenticServicePipeline and wait for completion.
    ...
    async fn review(&self, r2_url: &str, scenario: &DfyScenario) -> LLMReview {
        ...
        let prompt = format!(
            "You are reviewing a DFY service demo video generated by an AI agent pipeline.\n\
             Service: {name} ({slug})\n\
             Brief: \"{desc}\"\n\
             ...
             - How well it sells the DFY service to a prospect\n\
