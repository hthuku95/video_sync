...
pub fn extract_audio(input_file: &str, output_file: &str, format: &str) -> Result<String, String> {
...
pub fn add_audio(video_file: &str, audio_file: &str, output_file: &str) -> Result<String, String> {
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
        .arg(video_file)
...
pub fn normalize_audio(
    ...
    let filter = format!(
        "loudnorm=I={}:LRA={}:TP={}",
        target_lufs, loudness_range, true_peak
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn denoise_audio(
    ...
    let filter = format!(
        "afftdn=noise_floor={}:noise_reduction={}:track_noise={}",
        noise_floor,
        reduction,
        if track_noise { 1 } else { 0 }
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn audio_compand(
    ...
    let filter = format!(
        "compand=attacks={}:decays={}:points={}:soft-knee={}:gain={}",
        attacks, decays, points, soft_knee, gain
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn add_phaser(
    ...
    let filter = format!(
        ...
        in_gain, out_gain, delay, decay, speed, phaser_type
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn remove_clicks(
    ...
    let filter = format!(
        "adeclick=w={}:o={}:a={}:t={}",
        window, overlap, arorder, threshold
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn apply_stereo_widen(
    ...
    let filter = format!(
        ...
        delay_ms, feedback, crossfeed, drymix
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn audio_limiter(
    ...
    let filter = format!(
        "alimiter=level_in=1:level_out=1:limit={}:attack={}:release={}:asc={}",
        limit_linear,
        attack,
        release,
        if asc { 1 } else { 0 }
...
pub fn add_flanger(
    ...
    let filter = format!(
        "flanger=delay={}:depth={}:speed={}:shape={}",
        delay, depth, speed, shape
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn apply_audio_pulsator(
    ...
    let filter = format!(
        "apulsator=hz={}:amount={}:offset_l={}:offset_r={}:mode={}",
        hz, amount, offset_l, offset_r, mode
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn split_audio_channels(
    ...
    let filter = format!(
        "channelsplit=channel_layout={}[{ch}]",
        channel_layout,
        ch = channel
    );
    let mut command = Command::new("ffmpeg");
...
pub fn apply_crossfeed(
    ...
    let filter = format!(
        "crossfeed=strength={}:slope={}:level_in={}:level_out={}",
        strength, slope, level_in, level_out
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn filter_bandpass(
    ...
    let filter = format!(
        "bandpass=frequency={}:width={}:width_type={}",
        frequency, width, width_type
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn filter_bandreject(
    ...
    let filter = format!(
        "bandreject=frequency={}:width={}:width_type={}",
        frequency, width, width_type
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn boost_sub_bass(
    ...
    let filter = format!(
        "asubboost=dry={}:wet={}:cutoff={}:decay={}",
        d, w, cutoff, dc
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn remove_silence_simple(
    ...
    let filter = format!(
        "silenceremove=start_periods={}:start_duration={}:start_threshold={}",
        p, dur, th
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn soft_clip_audio(
    input_file: &str,
    output_file: &str,
    clip_type: &str,
    param: f64,
) -> Result<String, String> {
    let ct = if clip_type.is_empty() {
        "tanh"
    } else {
        clip_type
    };
...
pub fn blend_audio_streams(
    input_file: &str,
    secondary_file: &str,
    output_file: &str,
    duration: &str,
    dropout_transition: f64,
) -> Result<String, String> {
    ...
    let dt = if dropout_transition < 0.0 {
        2.0
    } else {
        dropout_transition
    };
...
pub fn generate_waveform_video(
    input_file: &str,
    output_file: &str,
    width: u32,
    height: u32,
...
pub fn apply_haas(
    ...
    let ms = if middle_source.is_empty() {
        "mid"
    } else {
        middle_source
    };
...
pub fn apply_aemphasis(
    ...
    let t = if emph_type.is_empty() {
        "cd"
    } else {
        emph_type
    }; // riaa, cd, 50fm, 75fm, 50kf, 75kf
    let filter = format!(
        "aemphasis=level_in={}:level_out={}:mode={}:type={}",
        level_in, level_out, m, t
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn apply_asetnsamples(
    ...
    let filter = format!(
        "asetnsamples=nb_out_samples={}:pad={}",
        n,
        if pad { 1 } else { 0 }
    );
    let mut command = Command::new("ffmpeg");
...
pub fn apply_apad(
    ...
    let mut parts = vec![format!(
        "packet_size={}",
        if packet_size == 0 { 4096 } else { packet_size }
    )];
    if pad_len > 0 {
        parts.push(format!("pad_len={}", pad_len));
    }
    if whole_len > 0 {
        parts.push(format!("whole_len={}", whole_len));
    }
    if pad_dur > 0.0 {
        parts.push(format!("pad_dur={}", pad_dur));
    }
    if whole_dur > 0.0 {
        parts.push(format!("whole_dur={}", whole_dur));
    }
...
pub fn dynamic_audio_normalize(
    input_file: &str,
    output_file: &str,
    frame_len: u32,
    gausssize: u32,
    ...
    let filter = format!(
        "dynaudnorm=f={}:g={}:p={}:m={}:r={}:n={}",
        fl,
        gs,
        p,
        mg,
        rms,
        if coupling { 1 } else { 0 }
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn multiband_compress(
    ...
    let default_params =
        "0.005 0.1 -47/-40 -34/-34 -17/-17 0/0 2 500 0.003 0.05 -47/-40 -34/-34 -17/-17 0/0 2";
...
pub fn denoise_audio_fft(
    ...
    let nf = if noise_floor == 0.0 {
        -25.0
    } else {
        noise_floor
    };
    let nr = if noise_reduction == 0.0 {
        12.0
    } else {
        noise_reduction
    };
...
pub fn apply_compensation_delay(
    input_file: &str,
    output_file: &str,
    mm: f64,
    cm: f64,
...
pub fn apply_surround_upmix(
    ...
    let filter = format!(
        "surround=chl_out={}:chl_in={}:level_in={}:level_out={}",
        out_layout, in_layout, li, lo
    );
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn visualize_cqt(
    ...
    let filter = format!(
        "[0:a]showcqt=size={}x{}:bar_h={}:axis_h={}[v]",
        w, h, bh, ah
    );
    let mut command = Command::new("ffmpeg");
    command
...
pub fn convert_audio_format(
    ...
    if !sample_fmts.is_empty() {
        parts.push(format!("sample_fmts={}", sample_fmts));
    }
    if !sample_rates.is_empty() {
        parts.push(format!("sample_rates={}", sample_rates));
    }
    if !channel_layouts.is_empty() {
        parts.push(format!("channel_layouts={}", channel_layouts));
    }
    let filter = if parts.is_empty() {
        "aformat=sample_fmts=s16".to_string()
    } else {
        format!("aformat={}", parts.join(":"))
    };
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn decode_hdcd(
    ...
    if disable_autoconvert {
        params.push("disable_autoconvert=1".to_string());
    }
    if process_stereo {
        params.push("process_stereo=1".to_string());
    }
    if force_pe {
        params.push("force_pe=1".to_string());
    }
    let filter = if params.is_empty() {
        "hdcd".to_string()
    } else {
        format!("hdcd={}", params.join(":"))
    };
    let mut command = Command::new("ffmpeg");
    command
        .arg("-i")
...
pub fn measure_audio_spectrum(
    ...
    let filter = format!(
        "[0:a]showspectrum=size={}x{}:mode={}:color={}[v]",
        w, h, m, c
    );
    let mut command = Command::new("ffmpeg");
    command
