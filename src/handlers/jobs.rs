...
pub async fn workflow_trace(
    ...
    let workflow = match workflow {
        Ok(Some(row)) => serde_json::json!({
            ...
            "lease_expires_at": row.try_get::<Option<chrono::DateTime<chrono::Utc>>, _>("lease_expires_at").ok().flatten().map(|d| d.to_rfc3339()),
            "cancel_requested": row.try_get::<Option<chrono::DateTime<chrono::Utc>>, _>("cancel_requested_at").ok().flatten().map(|d| d.to_rfc3339()),
            ...
            "updated_at": row.try_get::<Option<chrono::DateTime<chrono::Utc>>, _>("updated_at").ok().flatten().map(|d| d.to_rfc3339()),
            "last_heartbeat_at": row.try_get::<Option<chrono::DateTime<chrono::Utc>>, _>("last_heartbeat_at").ok().flatten().map(|d| d.to_rfc3339()),
