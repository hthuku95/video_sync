...
pub fn api_access_routes() -> Router {
    Router::new()
        ...
        .route(
            "/api/agency/subscribe/:id/unlock-spec",
            get(subscription_unlock_spec),
        )
        .route(
            "/api/agency/subscribe/:id/unlock",
            post(subscription_unlock),
        )
}
...
async fn create_subscription(
    ...
    let Some(tier) = tier_for(&req.tier) else {
        return Json(
            json!({"success": false, "error": "Invalid tier — must be 'starter' or 'pro'"}),
        );
    };
    ...
    let api_key = format!("vk_{}", hex::encode(rand::random::<[u8; 16]>()));
...
async fn subscription_unlock(
    ...
    let uuid = match uuid::Uuid::parse_str(&id) {
        ...
        Err(_) => {
            return (
                ...
                Json(json!({"success": false, "error": "Invalid subscription id"})),
            )
        }
    };
    ...
    .await
    {
        ...
        Ok(None) => {
            return (
                ...
                Json(json!({"success": false, "error": "Subscription not found"})),
            )
        }
        Err(e) => {
            return (
    ...
    if status == "active" {
        ...
        return (
            StatusCode::OK,
            Json(
                json!({"success": true, "api_key": key, "message": "Subscription is already active"}),
            ),
        );
    }
